...
void *requester_entryFunction(void *thd){
    ...
    for (int i = 0; i < reqG->var.GLOBALFILECOUNT; i++) {
        if(!pthread_mutex_trylock(&reqG->mut.lock[i])) { //if true, thread now owns lock
            ...
            pid_t tid = syscall(SYS_gettid); //gets ID for the service print
            printf("\n%d is servicing %s\n", tid, fileTobeServiced);
            
            pthread_mutex_lock(&reqG->mut.serviceLock); //lock service lock
            ...
            FILE* input_file = fopen(fileTobeServiced, "r"); //read from file
            if(!input_file) {
                printf("Error: invalid Input File\n");
            }
            char domainName[20];
            while(fgets(domainName, sizeof(domainName), input_file)) { //pull domains and place them in buffer
                domainName[strlen(domainName)-1] = '\0';
                strcpy(reqG->var.sharedBuffer[reqG->var.bufferIndex], domainName);
                reqG->var.bufferIndex++; //move to next line in buffer
            }
...
void *resolver_entryFunction(void *thd){
    ...
    if(!pthread_mutex_trylock(&reqG->mut.sharedBufferLock)) { //if true, thread now owns lock
        for(int i = 0; i < reqG->var.bufferIndex; i++) { //iterate through each line in the list
            ...
            if (dnslookup(domain, ip, sizeof(ip))) { //lookup IP address
                ...
                flag  = 1; //because invalid hostname
                printf("\n");
            } 
            else {
                printf("%s\n", ip); //print IP
            ...
            pthread_mutex_lock(&reqG->mut.resultsLock); //lock results lockk
                ...
                /* add some text */
                if (!flag) { //flag checks for invalid hostnames
                    fprintf(f, "%s, %s\n", domain, ip);
                }
                else {
                    fprintf(f, "%s,\n", domain);
...
int main (int argc, char *argv[]) {
    ...
    for (int i = 0; i < 10; i++) pthread_mutex_init(&gbls.mut.lock[i],NULL);
