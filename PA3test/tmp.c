...
struct globalVariables {
    ...
    int numb_req_threads;
    char *ptr_to_fileArray[10];
...
void *requester_entryFunction(void *thd) {
    ...
    struct globalVariables *td = thd; //since it is passed in as void

    for (int i = 0; i < td->GLOBALFILECOUNT; i++) {
        if (!pthread_mutex_trylock(&td->lock[i])) { //if true, thread now owns lock
            ...
            pthread_mutex_lock(&td->serviceLock);
                FILE *f = fopen(td->txtServiced, "a"); //serviced texts
                if (f == NULL) {
                ...
                fclose(f);
            pthread_mutex_unlock(&td->serviceLock);
            ...
            while(fgets(domainName, sizeof(domainName), input_file)) {
                ...
                strcpy(td->sharedBuffer[td->bufferIndex], domainName);
                td->bufferIndex++;
            }
            pthread_mutex_destroy(&td->lock[i]);
        }
    }
    return 0;
}
...
void *resolver_entryFunction(void *thd){
    struct globalVariables *td = thd;

    if(!pthread_mutex_trylock(&td->sharedBufferLock)) {
        for(int i = 0; i < td->bufferIndex; i++) {
            char domain[20]; //empty domain string
            char ip[20]; //empty IP string
            strcpy(domain, td->sharedBuffer[i]);
            ...
            pthread_mutex_lock(&td->resultsLock);
                FILE *f = fopen(td->txtResults, "a"); //append data to file
                if (f == NULL) {
                ...
                fclose(f);
            pthread_mutex_unlock(&td->resultsLock);
        }
    pthread_mutex_destroy(&td->sharedBufferLock);
    }
}
//--------------------------------------------------------------------------------------------------
int main (int argc, char *argv[]) {
    struct globalVariables thread_data;
    //serviced file
    strcpy(thread_data.txtServiced, argv[4]);
    // results file
    strcpy(thread_data.txtResults, argv[3]);
    //requester threads 
    thread_data.numb_req_threads = strtol(argv[1], NULL, 10); //number of request threads
    pthread_t rTID[thread_data.numb_req_threads];
    ...
    thread_data.GLOBALFILECOUNT = fileCount;
    thread_data.bufferIndex = 0;
    ...
    for (int i = 0; i <= thread_data.numb_req_threads; i++){
        thread_data.ptr_to_fileArray[i] = &f;
    }
    
    //create requester threads
    for (int i = 0; i < thread_data.numb_req_threads; i++){
        pthread_create(&rTID[i], NULL, requester_entryFunction, &thread_data);
    } 
    ...
    for (int i = 0; i < numb_resolver_threads; i++){
        pthread_create(&resolverThreads[i], NULL, resolver_entryFunction, &thread_data);
    }
